	...
	constructor() {
		...
		this.addOptionDefinitions({
			source: {
				...
				path: true,
				describe:
					'Source directory to recursively parse for KSS comments, homepage, and markup'
			},
			destination: {
				group: 'File locations:',
				string: true,
			...
			mask: {
				...
				default: '*.css|*.less|*.sass|*.scss|*.styl|*.stylus|*.yml|*.yaml'
			},
			clone: {
				group: 'Builder:',
				string: true,
			...
			extend: {
				...
				path: true,
				describe:
					'Location of modules to extend the templating system; see http://bit.ly/kss-wiki'
			},
			homepage: {
				group: 'Style guide:',
				string: true,
		...
		return new Promise((resolve, reject) => {
			...
			try {
				...
				if (typeof builderClass === 'function') {
					...
					return reject(
						new Error(
							'Unexpected value for "builder"; should be a path to a module or a JavaScript Class.'
						)
					);
				}
				...
				const supportedBuilders = [ 'builder/twig', 'builder/nunjucks' ];
				// istanbul ignore if
				if (supportedBuilders.indexOf(builderClass) > -1) {
					return reject(
						new Error(
							`The specified builder, "${builderClass}", is not relative to the current working directory.`
						)
					);
				} else {
			...
			if (!isCompatible) {
				return reject(
					new Error(
						'kss expected the builder to implement KssBuilderBase API version ' +
							kssBuilderAPI +
							'; version "' +
							builderAPI +
	...
	addOptions(options) {
		for (let key in options) {
			if (
				...
				[ 'logFunction', 'logErrorFunction' ].indexOf(key) === -1
			) {
			...
			.then((result) => {
				...
				if (!(result instanceof Error)) {
					return Promise.reject(
						new Error('This folder already exists: ' + destinationPath)
					);
				}
				...
				if (result.code === 'ENOENT') {
					return fs.copyAsync(builderPath, destinationPath, {
						clobber: true,
						filter: (filePath) => {
							...
							return new RegExp('^(?!.*\\' + path.sep + '(node_modules$|\\.))').test(
								relativePath
							);
						}
					});
	...
	prepare(styleGuide) {
		...
		sectionReferences.forEach((reference) => {
			...
			for (let i = 0; i < refParts.length - 1; i++) {
				...
				if (
					sectionReferences.indexOf(checkReference) === -1 &&
					newSections.indexOf(checkReference) === -1
				) {
					newSections.push(checkReference);
					// Add the missing section to the style guide.
					styleGuide.autoInit(false).sections({
					...
					.copyAsync(
						...
						path.join(this.options.destination, assetDirectory),
						{
							...
							filter: (filePath) => {
								// Only look at the part of the path inside the builder.
								let relativePath =
									path.sep + path.relative(this.options.builder, filePath);
								// Skip any files with a path matching: "/node_modules" or "/."
								return new RegExp(
				...
				fs
					...
					.catch((error) => {
						...
						if (this.options.verbose) {
							this.logError(
								new Error(
									'An error occurred when attempting to use the "extend" directory, ' +
										directory +
										': ' +
										error.message
								)
	...
	buildGuide(styleGuide, options) {
		...
		if (typeof this.templates.section === 'undefined') {
			readBuilderTask = readBuilderTask.then(() => {
				return readBuilderTemplate('section')
					.then((template) => {
		...
		if (typeof this.templates.item === 'undefined') {
			readBuilderTask = readBuilderTask.then(() => {
				return readBuilderTemplate('item')
					.then((template) => {
		...
		sections.forEach((section) => {
			...
			if (template.markup.search('^[^\n]+.(html|' + templateExtension + ')$') === -1) {
				...
				this.options.source.forEach((source) => {
					...
					findTemplates.push(
						glob(source + '/**/' + template.file).then(returnFilesAndSource)
					);
					findTemplates.push(
						glob(
							source +
								'/**/' +
								path.join(path.dirname(template.file), matchExampleFilename)
						).then(returnFilesAndSource)
					);
				});
				buildTasks.push(
					Promise.all(findTemplates)
						.then((globMatches) => {
		...
		return Promise.all(buildTasks)
			.then(() => {
				...
				sectionRoots.forEach((rootReference) => {
					buildPageTasks.push(
						this.buildPage(
							...
							options,
							rootReference,
							this.styleGuide.sections(rootReference + '.*')
						)
					);
				});
				...
				if (this.templates.item) {
					sections.forEach((section) => {
						buildPageTasks.push(
							this.buildPage('item', options, section.reference(), [ section ])
						);
					});
				}
		...
		return Promise.all(
			context.sections.map((section) => {
				...
				if (!section.markup) {
					...
					if (
						!this.options.markup &&
						path.extname(templateInfo.filename) === '.' + options.templateExtension
					) {
					...
					return markupTask
						...
						.then((template) => {
							section.example = templateRender(
								template,
								contextClone(exampleContext)
							);
			...
			if (templateName !== 'index') {
				...
				getHomepageText =
					typeof context.homepage !== 'undefined'
						...
						: Promise.all(
								...
								.then((globMatches) => {
									...
									if (this.options.verbose) {
										this.log(
											'   ...no homepage content found in ' +
												this.options.homepage +
												'.'
										);
									} else {
										this.log(
											'WARNING: no homepage content found in ' +
												this.options.homepage +
												'.'
										);
									}
   ...
   */
	createMenu(pageReference) {
		...
		return this.styleGuide.sections('x').map((rootSection) => {
			...
			menuItem.children = this.styleGuide
				.sections(rootSection.reference() + '.*')
				.slice(1)
				.map(toMenuItem);
