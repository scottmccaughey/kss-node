  ...
  toJSON() {
    ...
    for (let i = 0; i < this.meta.customPropertyNames.length; i++) {
      ...
      if (typeof this.custom(this.meta.customPropertyNames[i]) !== 'undefined') {
        returnObject[this.meta.customPropertyNames[i]] = this.custom(
          this.meta.customPropertyNames[i]
        );
      }
    }
  ...
  _propertyHelper(propertyName, objectConstructor, properties) {
    ...
    if (typeof properties === 'object') {
      ...
      properties.forEach(property => {
        let newProperty =
          property instanceof objectConstructor ? property : new objectConstructor(property);
