...
const parseColors = function(text) {
	...
	const regex = /^(?:(\S+)\s*:\s*)?([a-zA-Z]+|#[0-9a-f]{3}|#(?:[0-9a-f]{2}){2,4}|(?:rgb|hsl)a?\((?:-?\d+%?[,\s]+){2,3}\s*[\d\.]+%?\))(?:\s*-\s*(.*))?$/gim;
			...
			if (paragraphs.length === 0) {
				...
				for (let j = 0; j < numModifierLines; j += 1) {
					if (newSection.modifiers[j].match(/^\s*.+?\s+\-\s/g)) {
						...
						newSection.modifiers[lastModifier] +=
							' ' + newSection.modifiers[j].replace(/^\s+|\s+$/g, '');
						// We will strip this blank line later.
						newSection.modifiers[j] = '';
					}
				}
				// Remove any blank lines added.
				newSection.modifiers = newSection.modifiers.filter((line) => {
					return line !== '';
				});
...
const findCommentBlocks = function(input) {
	...
	for (let i = 0; i < lines.length; i += 1) {
		...
		if (
			insideSingleBlock ||
			((insideMultiBlock || insideDocblock) && line.match(commentRegex.multiFinish))
		) {
